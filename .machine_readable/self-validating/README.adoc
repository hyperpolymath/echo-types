...
This is echo-types' copy of the shared estate k9 template set
(reference: nextgen-typing / maa-framework `.machine_readable/self-validating/`),
adapted only in repo-identity fields. The examples point at this repo's
real validation targets: `agda -i proofs/agda proofs/agda/All.agda`
(full suite), `agda -i proofs/agda proofs/agda/Smoke.agda` (headline
pins), `scripts/kernel-guard.sh` and `tools/check-guardrails.sh`
(guardrails). Added at the 2026-06-12 governance checkpoint.
