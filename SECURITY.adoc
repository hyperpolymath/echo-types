...
Expect an acknowledgement within 72 hours and an initial assessment
within 7 days.
...
* No `+believe_me+` in distinctness arguments. CI checks for these
patterns.
* Imports are pinned in `+echo-types.agda-lib+`; updates are reviewed.
* Container images use Chainguard distroless bases (when
`+stapeln.toml+` lands a green build).

There is no networked attack surface — echo-types is a proof library,
not a service. Report concerns about proof soundness to the same address
as security issues.
