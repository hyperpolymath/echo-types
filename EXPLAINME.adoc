...
== Applied prototype hook

`docs/echo-types/prototypes/warrant_debugger_prototype.jsx` is the
current doc-side React sketch for the warrant/debugging surface. It
belongs below the semantic-fibre vocabulary, not in the README
headline: it shows an empty-fibre contradiction and cost-disclosed
repair moves (weaken, widen, soft support) for readers who want to see
how "warrant" might appear in tooling. It is not an Agda proof artefact
and does not change the kernel/status claims.
...
* `docs/echo-types/prototypes/README.adoc` — doc-side UI prototype index; currently the warrant debugger sketch.
