...
. *Graded degradation with composition.* `EchoGraded.degrade-comp`
  ...
  `_≤g_`. The per-decoration composition rung adds `≤g-prop` (the
  order is propositional), `≤g-⊔g-{left, right, univ}` (the order
  has a categorical join), and `degrade-compose` /
  `degrade-via-join` — any two ways of degrading through a common
  upper bound agree on the underlying echo. K-provenance has a
  natural homomorphism story ("evaluate in a simpler semiring"),
  but a semiring homomorphism is a different mathematical object
  from a propositional pre-order with a typed lowering family
  indexed over its proofs.
