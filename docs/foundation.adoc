...
. *CI Agda exact pin* — *reproducible pin IMPLEMENTED 2026-05-18;
  ...
  toolchain; an *additive* `flake-check` CI job (`guix flake check`)
  exercises it. This is honest about its boundary: the dev
  environment has no `guix`, so the flake was authored designed-correct
  ...
  nixpkgs/Agda to bump. (Cross-stack: a devShell-only `flake.guix`
